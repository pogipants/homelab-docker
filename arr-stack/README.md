...
mkdir -p /apps/runtime/{gluetun,radarr,sonarr,prowlarr,bazarr,recyclarr,sabnzbd,decluttarr}
mkdir -p /mnt/bigshare/Media/{movies,shows}
mkdir -p /mnt/bigshare/downloads

# Only if using a separate cache directory
mkdir -p /apps/cache

# Only if using COMPOSE_PROFILES=requests
mkdir -p /apps/runtime/jellyseerr

# Only if using COMPOSE_PROFILES=music
mkdir -p /apps/runtime/lidarr /mnt/media/music

# Only if using COMPOSE_PROFILES=audiobooks
mkdir -p /apps/runtime/bookshelf /mnt/media/audiobooks

# Only if using COMPOSE_PROFILES=subtitles
mkdir -p /apps/runtime/subgen/models
```
